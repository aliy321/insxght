...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`blogs_populated_authors\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text,
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`blogs_populated_authors_order_idx\` ON \`blogs_populated_authors\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`blogs_populated_authors_parent_id_idx\` ON \`blogs_populated_authors\` (\`_parent_id\`);`,
  )
  ...
  `)
  await db.run(sql`CREATE INDEX \`blogs_hero_image_idx\` ON \`blogs\` (\`hero_image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`blogs_rels_blogs_id_idx\` ON \`blogs_rels\` (\`blogs_id\`);`)
  await db.run(
    sql`CREATE INDEX \`blogs_rels_categories_id_idx\` ON \`blogs_rels\` (\`categories_id\`);`,
  )
  await db.run(sql`CREATE INDEX \`blogs_rels_users_id_idx\` ON \`blogs_rels\` (\`users_id\`);`)
  await db.run(sql`CREATE TABLE \`_blogs_v_version_populated_authors\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_uuid\` text,
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`_blogs_v_version_populated_authors_order_idx\` ON \`_blogs_v_version_populated_authors\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_blogs_v_version_populated_authors_parent_id_idx\` ON \`_blogs_v_version_populated_authors\` (\`_parent_id\`);`,
  )
  ...
  await db.run(
    sql`CREATE INDEX \`_blogs_v_version_version_hero_image_idx\` ON \`_blogs_v\` (\`version_hero_image_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_blogs_v_version_meta_version_meta_image_idx\` ON \`_blogs_v\` (\`version_meta_image_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_blogs_v_version_version_slug_idx\` ON \`_blogs_v\` (\`version_slug\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_blogs_v_version_version_updated_at_idx\` ON \`_blogs_v\` (\`version_updated_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_blogs_v_version_version_created_at_idx\` ON \`_blogs_v\` (\`version_created_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_blogs_v_version_version__status_idx\` ON \`_blogs_v\` (\`version__status\`);`,
  )
  ...
  await db.run(sql`CREATE INDEX \`_blogs_v_autosave_idx\` ON \`_blogs_v\` (\`autosave\`);`)
  ...
  await db.run(
    sql`CREATE INDEX \`_blogs_v_rels_blogs_id_idx\` ON \`_blogs_v_rels\` (\`blogs_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_blogs_v_rels_categories_id_idx\` ON \`_blogs_v_rels\` (\`categories_id\`);`,
  )
  await db.run(
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`projects_populated_authors_order_idx\` ON \`projects_populated_authors\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`projects_populated_authors_parent_id_idx\` ON \`projects_populated_authors\` (\`_parent_id\`);`,
  )
  ...
  await db.run(
    sql`CREATE INDEX \`projects_rels_projects_id_idx\` ON \`projects_rels\` (\`projects_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`projects_rels_categories_id_idx\` ON \`projects_rels\` (\`categories_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`projects_rels_users_id_idx\` ON \`projects_rels\` (\`users_id\`);`,
  )
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`_projects_v_version_populated_authors_order_idx\` ON \`_projects_v_version_populated_authors\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_projects_v_version_populated_authors_parent_id_idx\` ON \`_projects_v_version_populated_authors\` (\`_parent_id\`);`,
  )
  ...
  await db.run(
    sql`CREATE INDEX \`_projects_v_version_version_hero_image_idx\` ON \`_projects_v\` (\`version_hero_image_id\`);`,
  )
  await db.run(
  ...
  await db.run(
    sql`CREATE INDEX \`_projects_v_rels_users_id_idx\` ON \`_projects_v_rels\` (\`users_id\`);`,
  )
  await db.run(sql`CREATE TABLE \`media\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`alt\` text,
  	\`caption\` text,
  	\`prefix\` text DEFAULT 'hyperfuse/',
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`url\` text,
  ...
  await db.run(sql`CREATE TABLE \`homepage_offer_offerings\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`homepage_offer_offerings_order_idx\` ON \`homepage_offer_offerings\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`homepage_offer_offerings_parent_id_idx\` ON \`homepage_offer_offerings\` (\`_parent_id\`);`,
  )
  await db.run(sql`CREATE TABLE \`homepage\` (
  	...
  	\`featured_projects_cta_text\` text,
  	\`featured_projects_cta_link\` text,
  	\`outro_title\` text,
  	\`insights_title\` text,
  	\`insights_description\` text,
  	\`insights_cta_text\` text,
  	\`insights_cta_link\` text,
  	\`updated_at\` text,
  	\`created_at\` text,
  	FOREIGN KEY (\`hero_hero_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`projects_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(
    sql`CREATE INDEX \`homepage_hero_hero_hero_image_idx\` ON \`homepage\` (\`hero_hero_image_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`homepage_projects_projects_image_idx\` ON \`homepage\` (\`projects_image_id\`);`,
  )
  ...
  await db.run(
    sql`CREATE INDEX \`homepage_rels_projects_id_idx\` ON \`homepage_rels\` (\`projects_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`homepage_rels_posts_id_idx\` ON \`homepage_rels\` (\`posts_id\`);`,
  )
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`blogs_populated_authors\`;`)
  await db.run(sql`DROP TABLE \`blogs\`;`)
  await db.run(sql`DROP TABLE \`blogs_rels\`;`)
  await db.run(sql`DROP TABLE \`_blogs_v_version_populated_authors\`;`)
  ...
  await db.run(sql`DROP TABLE \`homepage_offer_offerings\`;`)
